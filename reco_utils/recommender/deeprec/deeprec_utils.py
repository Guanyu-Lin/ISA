...
def create_hparams(flags):
    ...
    return tf.contrib.training.HParams(
        ...
        is_random=flags["is_random"] if "is_random" in flags else True,
        ...
        contrastive_l2=flags["contrastive_l2"] if "contrastive_l2" in flags else 0.0000,
        ...
        components=flags["components"] if "components" in flags else '',
...
def hit_score(y_true, y_score, k=10):
    ...
    argsort = np.argsort(y_score, kind='heapsort')[::-1][:k]
    for idx in argsort:
...
def cal_metric(labels, preds, metrics):
    ...
    for metric in metrics:
        if metric == "auc":
            ...
            mean_mrr = np.mean(
                [
                    mrr_score(each_labels, each_preds) # each batch
