...
class SequentialBaseModel(BaseModel):
    ...
    def _build_graph(self):
        ...
        with tf.variable_scope("sequential") as self.sequential_scope:
            ...
            model_output_u_i = self._build_seq_graph()
            
            logit_u_i = self._fcn_net(model_output_u_i, hparams.layer_sizes, scope="logit_fcnui")
        ...
        for batch_data_input in file_iterator:
           # if self.step == 0:
            ...
            if batch_data_input:
                ...
                self.step += 1
                if self.step % self.hparams.show_step == 0:
                    print(
                        "self.step {0:d} , total_loss: {1:.4f}, data_loss: {2:.4f}".format(
                            self.step, step_loss, step_data_loss
                        )
                    )
                   # print("Gradient A", step_gradient)
                    if self.hparams.visual_type == 'epoch':
                        ...
                        tb.add_scalar('loss', step_loss, self.step)
               # if self.step % 600 == 0:  break

                if self.hparams.visual_type == 'self.step':
                    if self.step % self.hparams.visual_step == 0:
                        if vm != None:
                            vm.step_update_line('loss', step_loss)
                        #  tf.summary.scalar('loss',step_loss)
                        tb.add_scalar('loss', step_loss, self.step)
                        ...
                        for vs in valid_res:
                            ...
                            tb.add_scalar(vs.replace('@', '_'), valid_res[vs], self.step)
    ...
    def fit(
        ...
        for epoch in range(1, self.hparams.epochs + 1):
            ...
            valid_resA = self.run_weighted_eval(valid_fileA, valid_num_ngs)
    ...
    def run_weighted_eval_userSeq(self, filename, num_ngs, calc_mean_alpha=False):
        """Evaluate the given file and returns some evaluation metrics.
        ...
        for batch_data_input in self.iterator.load_data_from_file(
            ...
            if batch_data_input:
                if not calc_mean_alpha:
                    step_item, step_pred, step_labels = self.eval_with_item(load_sess, batch_data_input)
    ...
    def run_weighted_eval(self, filename, num_ngs, calc_mean_alpha=False):
        ...
        for batch_data_input in self.iterator.load_data_from_file(
            ...
            if batch_data_input:
                if not calc_mean_alpha:
                    # step_user, step_pred, step_labels, window_mask, initial_key_mask, query_mask, combine_mask, real_mask = self.eval_with_user(load_sess, batch_data_input)
    ...
    def eval_with_user(self, sess, feed_dict):
        ...
        # return sess.run([self.iterator.users, self.predA, self.iterator.labels, self.window_mask, self.initial_key_mask, self.query_mask, self.combine_mask, self.mask], feed_dict=feed_dict)
